...
SECRET_KEY = os.getenv("JWT_SECRET", "bambu-filament-linux-jwt-secret-key-1.2.0")
ALGORITHM = "HS256"
...
class DeviceStatus(Base):
    ...
    # 系统信息
    free_heap = Column(Integer, nullable=True)           # 剩余内存
...
class HeartbeatData(BaseModel):
    ...
    free_sketch_space: Optional[int] = None
...
GITHUB_FIRMWARE_API = "https://api.github.com/repos/fabie250/siheng/releases/latest"
...
# 本地手动上传的固件存在 local 子目录（和 GitHub 在线更新独立）
LOCAL_FIRMWARE_DIR = os.path.join(FIRMWARE_DIR, "local")
os.makedirs(LOCAL_FIRMWARE_DIR, exist_ok=True)
FIRMWARE_INFO_PATH = os.path.join(LOCAL_FIRMWARE_DIR, "version.json")
...
def scale_ota_check_get(source: str = "", user: User = Depends(verify_script_api_key)):
    """
    检查固件更新
    source=local: 只检查本地上传的固件
    source=github: 忽略本地固件，直接去 GitHub 拉取
    不传 source: 优先本地，没有再去 GitHub
    """
    # 如果指定了 github，直接去 GitHub 拉取
    if source != "github":
        # 优先检查本地上传的固件
        info = get_firmware_info()
        filepath = os.path.join(LOCAL_FIRMWARE_DIR, info.get("filename", "firmware.bin"))
        if os.path.exists(filepath) and info.get("version"):
            local_version = info["version"].replace("v", "")
            download_url = f"/api/scale/firmware/local/{info.get('filename', 'firmware.bin')}"
            return {"status": "success", "latest_version": local_version, "download_url": download_url, "source": "local"}

    # 如果指定了 local 但本地没有，返回错误
    if source == "local":
        return {"status": "error", "message": "本地没有上传固件"}
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=5) as resp:
            if resp.status == 200:
                ...
                return {"status": "success", "latest_version": latest_ver, "download_url": download_url, "source": "github"}
    except Exception:
        pass
    return {"status": "error", "message": "无法获取最新固件版本"}
...
def push_ota_to_device(data: OtaPushBody, user: User = Depends(verify_script_api_key), db: Session = Depends(get_db)):
    ...
    return {"status": "success", "message": "OTA 更新指令已下发，设备将在5秒内重启并拉取更新"}

# ---------- 固件信息 (网页端用) ----------
@app.get("/api/scale/firmware/info", tags=["智能称重节点"])
def get_firmware_info_endpoint(user: User = Depends(verify_script_api_key)):
    info = get_firmware_info()
...
async def upload_firmware(
    ...
    filepath = os.path.join(LOCAL_FIRMWARE_DIR, filename)
    ...
    return {"status": "success", "message": f"固件 v{version} 上传成功（本地固件）", "info": info}
...
# 本地固件下载（独立路径）
@app.get("/api/scale/firmware/local/{filename}", tags=["智能称重节点"])
def download_local_firmware(filename: str):
    filepath = os.path.join(LOCAL_FIRMWARE_DIR, filename)
    if not os.path.exists(filepath):
        raise HTTPException(status_code=404, detail="本地固件文件不存在")
    return FileResponse(filepath, media_type="application/octet-stream", filename=filename)
...
# 切换 OTA 分区
@app.post("/api/scale/device/switch_partition", tags=["智能称重节点"])
def switch_device_partition(device_id: str = "scale_001", user: User = Depends(verify_script_api_key), db: Session = Depends(get_db)):
    cmd = DeviceCommand(device_id=device_id, command="switch_partition", params_json=json.dumps({}), user_id=user.id)
    db.add(cmd); db.commit()
    return {"status": "queued", "message": "切换分区指令已下发，设备将在5秒内重启并切换到另一个分区"}
...
def scale_heartbeat(data: HeartbeatData, user: User = Depends(verify_script_api_key), db: Session = Depends(get_db)):
    ...
    # 系统信息
    status_row.free_heap = getattr(data, "free_heap", None)
...
def get_device_status(device_id: str = "scale_001", user: User = Depends(verify_script_api_key), db: Session = Depends(get_db)):
    ...
    return {
        ...
        # 系统信息
        "free_heap": status_row.free_heap,
...
if __name__ == "__main__":
    ...
    print("=" * 72)
    print("      丝管 3D 打印耗材资产管理系统 - Linux 服务端 v1.2.0")
    print("=" * 72)
    print(f"📦 数据库类型: MySQL ({DB_HOST}:{DB_PORT}/{DB_NAME})")
    print(f"🏠 本机内网IP: {local_ip}")
    if pub_ip:
        print(f"🌐 检测到公网: {pub_ip}")
    print("-" * 72)
    print(f"🚀 服务端启动中... 监听 {SERVER_HOST}:{SERVER_PORT}")
    print(f"📌 本机浏览器访问 : http://127.0.0.1:{SERVER_PORT}")
    print(f"📌 局域网/ESP32地址: http://{local_ip}:{SERVER_PORT}")
    if pub_ip:
        print(f"📌 公网远程访问   : http://{pub_ip}:{SERVER_PORT} (请确保防火墙已放行)")
    print("=" * 72 + "\n")

    uvicorn.run(app, host=SERVER_HOST, port=SERVER_PORT, proxy_headers=True, forwarded_allow_ips="*")